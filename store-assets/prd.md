...
| Aspect | Chrome (MV3) | Firefox (MV2) | Safari (MV2) |
|---|---|---|---|
| Manifest version | Manifest V3 | Manifest V2 | Manifest V2 |
| API namespace | `chrome.*` | `browser.*` | `browser.*` |
| Host permissions | `host_permissions` field | `permissions` field | `permissions` field |
| Messaging pattern | `sendResponse` + `return true` | Returns Promises (native pattern) | Same as Firefox |
| Gecko settings | N/A | `browser_specific_settings.gecko` with gecko ID and `strict_min_version` | Removed |
| ETP handling | N/A | Early classification in `onBeforeRequest` + `onErrorOccurred` to detect brokers blocked by Enhanced Tracking Protection | N/A |
| Add-on persistence | Persistent (installed from store or unpacked) | Temporary add-ons reset on browser close | Requires Xcode-built `.app` |
