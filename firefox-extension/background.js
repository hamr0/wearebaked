...
/* ── Ensure domain entry exists (for early classification before ETP blocks) ── */
function ensureDomainEntry(domain, tabId) {
  const classification = classifyDomain(domain, null);
  ...
  if (!traffic.domains[domain]) {
    traffic.domains[domain] = {
      ...
      classification,
      recentTimestamps: [],
      isNew: !traffic.sessionDomains[domain],
      responseTimeTotal: 0,
      responseTimeCount: 0,
      bytesReceived: 0,
      bytesSent: 0,
    ...
    if (!traffic.sessionDomains[domain]) traffic.sessionDomains[domain] = true;
  }

  // Track third-party + broker badge
  const tabInfo = traffic.tabs[tabId];
  if (tabInfo) {
    const tabDomain = tabInfo.domain;
    if (isThirdParty(domain, tabDomain)) {
      traffic.domains[domain].thirdPartyOn[tabDomain] = true;
      tabInfo.thirdParties[domain] = true;
      if (classification.brokerName) {
        tabInfo.brokerCompanies[classification.brokerName] = true;
        updateBadge(tabId, Object.keys(tabInfo.brokerCompanies).length);
      }
    }
  }
}
...
browser.webRequest.onBeforeRequest.addListener(
  (details) => {
    ...
    if (details.type === 'main_frame') {
      ...
      // Early classify — catches domains before Firefox ETP may block them
      const domain = extractDomain(details.url);
...
/* ── onErrorOccurred: cleanup + catch ETP-blocked requests ── */
